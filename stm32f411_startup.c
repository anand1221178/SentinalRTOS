...
extern uint32_t _estack; //initial top of the stack -> value is loaded into the main stack ptr on startup
extern uint32_t _etext; //End of exec code section and start of data section stroed in flash -> use this as a reference pt. for copying initialised data from FLASH to SRAM
extern uint32_t _sdata; //Start addr of data section in SRAM
extern uint32_t _edata; //end addr of data section in SRAM
extern uint32_t _sbss; //Start addr of bss section in SRAM
extern uint32_t _ebss; //End addr of bss section in SRAM
...
/* __attribute__((weak,alias("Default_Handler"))) makes each handler weakly linked and aliases it to a function named Default_Handler ,it allows these handlers to be overrridden by defined handlers with teh same name elsewhere
    __attribute__ : use this keyword to tell the compiler that the declartion its applied to has certain properties that affect how its treaeted by the linerk and potentiallyat runtime.
    weak: doesnt prevetn the linker from using another symbol of the same name with a stronger  linkage -> we do this so we can define default handlers in our tsratup file, which app specific handlers can override w/o modifying the startup file
    alias(...) : means eg. NMI handler is not just weak but its also an alias for the default handler function -> means if intterupt occurs and if specfic handler has not been defined it will use the Default Handler in its place
*/
...
uint32_t vector_tbl[] __attribute__((section(".isr_vector_tbl")))={
  ...
  0, //These zeros act as placeholders for the positions corresponding to interrupts not supported by our specific microcontroller variant
  0,
  0,
  0,
...
/* Universal fallback for any interrupt req for which a specific handler has not been implemented*/
...
/* Reset Handler */
/*The job of this handler is to prepare the system before executing the main application*/
void Reset_Handler(void)
{
	// Calculate the sizes of the .data and .bss sections
    /* Subtract the address of the start of the section from the address of the end -> this size if used to copy initialised data from FLASH to SRAM, bss size is used to 0 out the section in SRAM*/
	...
	{
		/* copy the data word by word -> data is copied over then both ptrs are incremented to the next wod*/
